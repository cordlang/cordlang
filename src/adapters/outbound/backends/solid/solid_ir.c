/*
 * Solid IR-2 codegen: pure IrNode walkers (no ir_origin / AST for body emission).
 */
...
static void gen_ir_interpolation(StrBuf *sb, IrNode *node, int depth, GenCtx *ctx);

static void solid_track_signal(GenCtx *ctx, const char *name) {
  if (!ctx || !name || !*name || ctx->n_signals >= 32) return;
  for (int i = 0; i < ctx->n_signals; i++)
    if (strcmp(ctx->signals[i], name) == 0) return;
  snprintf(ctx->signals[ctx->n_signals], sizeof(ctx->signals[0]), "%s", name);
  ctx->n_signals++;
}

/* Rewrite bare signal identifiers to signal() for Solid JSX/handlers. */
static void solid_emit_expr(StrBuf *sb, const char *expr, GenCtx *ctx) {
  if (!expr) {
    sb_append(sb, "undefined");
    return;
  }
  if (!ctx || ctx->n_signals == 0) {
    sb_append(sb, expr);
    return;
  }
  const char *p = expr;
...
typedef enum { RK_PAGE = 0, RK_COMPONENT = 1, RK_LAYOUT = 2 } SolidKind;

#define SOLID_MAX_UNITS 64
#define SOLID_MAX_USED 24
#define SOLID_MAX_ROUTES 64
...
typedef struct {
  SolidUnit units[SOLID_MAX_UNITS];
  int n_units;
  IrNode *routes[SOLID_MAX_ROUTES];
  int n_routes;
...
static int route_is_lazy_ir(SolidProject *proj, IrNode *route) {
  if (!route) return 0;
...
static SolidUnit *project_find_unit(SolidProject *p, const char *name) {
  if (!name) return NULL;
...
static void project_partition_from_ir(SolidProject *p, IrProgram *ir) {
  memset(p, 0, sizeof(*p));
  if (!ir || !ir->root) return;
  IrNode *root = ir->root;

  IrNode *raw_comps[SOLID_MAX_UNITS];
  ...
  for (size_t i = 0; i < root->n_kids; i++) {
    ...
    if (c->kind == IR_COMPONENT) {
      if (n_comp < SOLID_MAX_UNITS) raw_comps[n_comp++] = c;
      else p->truncated = 1;
    } else if (c->kind == IR_LAYOUT) {
      if (n_layout < 16) raw_layouts[n_layout++] = c;
      else p->truncated = 1;
    } else if (c->kind == IR_ROUTE) {
      if (p->n_routes < SOLID_MAX_ROUTES) p->routes[p->n_routes++] = c;
      else p->truncated = 1;
  ...
  for (int i = 0; i < n_comp; i++) {
    if (p->n_units >= SOLID_MAX_UNITS) {
      p->truncated = 1;
      break;
    }
    SolidUnit *u = &p->units[p->n_units++];
    memset(u, 0, sizeof(*u));
  ...
  if (p->truncated) {
    fprintf(stderr,
            "error: project exceeds Solid backend limits "
            "(max %d units, %d routes) — split the app or raise limits\n",
            SOLID_MAX_UNITS, SOLID_MAX_ROUTES);
  }
}

static void import_path_between(const SolidUnit *from, const SolidUnit *to,
                                char *out, size_t n) {
...
static void gen_ir_element(StrBuf *sb, IrNode *node, int depth, GenCtx *ctx) {
  ...
  if (is_pascal_case(tag)) {
    ...
    for (size_t i = 0; i < node->n_kids; i++) {
      ...
      if (child->kind == IR_ATTR && child->name) {
        ...
        solid_event[0] = 'o';
        solid_event[1] = 'n';
        solid_event[2] = (char)toupper((unsigned char)child->name[0]);
        strncpy(solid_event + 3, child->name + 1, sizeof(solid_event) - 4);
        solid_event[sizeof(solid_event) - 1] = '\0';
        if (needs_js_arrow(child->value)) {
          sb_appendf(sb, " %s={() => ", solid_event);
          solid_emit_expr(sb, child->value, ctx);
          sb_append(sb, "}");
        } else {
          sb_appendf(sb, " %s={", solid_event);
          solid_emit_expr(sb, child->value, ctx);
  ...
  for (size_t i = 0; i < node->n_kids; i++) {
    ...
    if (child->kind == IR_EVENT && child->name && child->value) {
      ...
      solid_event[0] = 'o';
      solid_event[1] = 'n';
      solid_event[2] = (char)toupper((unsigned char)event[0]);
      strncpy(solid_event + 3, event + 1, sizeof(solid_event) - 4);
      solid_event[sizeof(solid_event) - 1] = '\0';
      if (strcmp(event, "submit") == 0) {
        if (needs_js_arrow(handler)) {
          sb_appendf(sb, " %s={(e) => { e.preventDefault(); ", solid_event);
          solid_emit_expr(sb, handler, ctx);
          sb_append(sb, "; }}");
        } else {
          sb_appendf(sb, " %s={(e) => { e.preventDefault(); ", solid_event);
          solid_emit_expr(sb, handler, ctx);
...
static void gen_ir_children(StrBuf *sb, IrNode *node, int depth, GenCtx *ctx) {
  for (size_t i = 0; i < node->n_kids; i++) {
    ...
    if (child->kind == IR_FOR) {
      ...
      gen_ir_interpolation(sb, child, depth, ctx);
...
static void gen_ir_interpolation(StrBuf *sb, IrNode *node, int depth, GenCtx *ctx) {
  ...
  if (node->value && node->n_kids == 0) {
    sb_indent(sb, depth);
    sb_append(sb, "{");
    solid_emit_expr(sb, node->value, ctx);
    sb_append(sb, "}\n");
    return;
  }
  if (node->n_kids > 0) {
...
static void gen_ir_node(StrBuf *sb, IrNode *node, int depth, GenCtx *ctx) {
  ...
  switch (node->kind) {
    ...
    case IR_HOOK:
      ...
      break;
    case IR_INTERP:
      gen_ir_interpolation(sb, node, depth, ctx);
      break;
    case IR_TEXT:
...
static void gen_component_fn_ir(StrBuf *sb, IrNode *def, const char *name,
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (c->name && strcmp(c->name, "__states__") == 0) {
      for (size_t j = 0; j < c->n_kids; j++) {
        ...
        sb_appendf(sb, "  const [%s, %s] = createSignal(", st->name, setter);
        emit_js_literal(sb, st->value ? st->value : "null");
        sb_append(sb, ");\n");
        solid_track_signal(ctx, st->name);
        ctx->use_state = 1;
      }
    } else if (c->name) {
      char setter[128];
      ...
      sb_appendf(sb, "  const [%s, %s] = createSignal(", c->name, setter);
      emit_js_literal(sb, c->value ? c->value : "null");
      sb_append(sb, ");\n");
      solid_track_signal(ctx, c->name);
      ctx->use_state = 1;
    }
  }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    sb_appendf(sb, "  const [%s, dispatch] = /* reducer */(%s, %s);\n", c->value,
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (ir_hook_is(c, "ref") && c->value) {
      sb_appendf(sb, "  const %s = /* ref */(", c->value);
      if (c->value2)
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (ir_hook_is(c, "transition")) {
      sb_appendf(sb, "  const [%s, %s] = /* transition */();\n",
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (c->kind == IR_COMPUTED && c->name) {
      sb_appendf(sb, "  const %s = createMemo(() => (", c->name);
      solid_emit_expr(sb, c->value ? c->value : "null", ctx);
      sb_append(sb, "));\n");
      solid_track_signal(ctx, c->name);
      ctx->use_memo = 1;
    }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    sb_appendf(sb, "  const %s = /* callback */(%s, [%s]);\n", c->value, fn, deps);
    ctx->use_callback = 1;
  }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    int is_fn = (fn[0] == '(' || strncmp(fn, "function", 8) == 0 ||
                 strstr(fn, "=>") != NULL);
    if (is_fn) {
      sb_appendf(sb, "  const %s = /* effectEvent */(%s);\n", c->value, fn);
    } else {
      int simple_id = 1;
      ...
      if (simple_id)
        ...
        sb_appendf(sb, "  const %s = /* effectEvent */((..._args) => { %s; });\n",
                   c->value, fn);
    }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (get_server && get_server[0] && strcmp(get_server, "null") != 0)
      sb_appendf(sb, "  const %s = /* store */(%s, %s, %s);\n",
                 c->value, sub, get_snap, get_server);
    else
      sb_appendf(sb, "  const %s = /* store */(%s, %s);\n", c->value,
                 sub, get_snap);
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    snprintf(set_load, sizeof(set_load), "set%c%sLoading",
             (char)toupper((unsigned char)nm[0]), nm + 1);
    sb_appendf(sb, "  const [%s, %s] = createSignal(null);\n", nm, set_n);
    sb_appendf(sb, "  const [%sError, %s] = createSignal(null);\n", nm, set_err);
    sb_appendf(sb, "  const [%sLoading, %s] = createSignal(true);\n", nm, set_load);
    solid_track_signal(ctx, nm);
    {
      char errn[128], loadn[128];
      snprintf(errn, sizeof(errn), "%sError", nm);
      snprintf(loadn, sizeof(loadn), "%sLoading", nm);
      solid_track_signal(ctx, errn);
      solid_track_signal(ctx, loadn);
    }
    sb_append(sb, "  createEffect(() => {\n");
...

  {
    ...
    for (int bi = 0; bi < n_binds; bi++) {
      ...
      sb_appendf(sb, "  const [%s, %s] = createSignal(\"\");\n", binds[bi], setter);
      solid_track_signal(ctx, binds[bi]);
      ctx->use_state = 1;
    }
  }
...
static char *gen_unit_module_ir(SolidProject *proj, SolidUnit *u) {
  GenCtx ctx = {0};
...

  {
    const char *hooks[32];
    int nh = 0;
    if (u->use_state || ctx.use_state) hooks[nh++] = "createSignal";
    if (u->use_memo || ctx.use_memo) hooks[nh++] = "createMemo";
    if (u->use_effect || ctx.use_effect) hooks[nh++] = "createEffect";
    ...
    if (u->use_portal || ctx.use_portal) hooks[nh++] = "Portal";
    if (u->use_error_boundary || ctx.use_error_boundary)
      hooks[nh++] = "ErrorBoundary";
    if (nh > 0) {
  ...
  if (u->use_link || u->use_outlet || ctx.use_params || ctx.use_navigate) {
    ...
    sb_append(&out, " } from '@solidjs/router';\n");
  }
...
static char *gen_contexts_module_ir(SolidProject *proj) {
  if (proj->n_contexts == 0) return NULL;
  StrBuf out;
  sb_init(&out);
  sb_append(&out, "import { createContext } from 'solid-js';\n\n");
  for (int i = 0; i < proj->n_contexts; i++) {
...
static SolidUnit *find_layout_for_route_ir(SolidProject *proj, IrNode *route) {
...
static char *gen_app_module_ir(SolidProject *proj) {
  StrBuf out;
  sb_init(&out);
  ...
  if (proj->n_routes > 0 || proj->has_router) {
    ...
    if (any_lazy_route)
      sb_append(&out, "import { lazy, Suspense } from 'solid-js';\n");
  ...
  if (proj->n_routes > 0) {
    ...
    int route_done[SOLID_MAX_ROUTES];
    memset(route_done, 0, sizeof(route_done));

    for (int r = 0; r < proj->n_routes; r++) {
      if (route_done[r]) continue;
      SolidUnit *layout = find_layout_for_route_ir(proj, proj->routes[r]);
      if (layout) {
        sb_appendf(&out, "        <Route path=\"/\" component={%s}>\n",
                   layout->name);
        for (int r2 = 0; r2 < proj->n_routes; r2++) {
          ...
          sb_appendf(&out, "          <Route path=\"%s\" component={%s} />\n",
                     path, comp);
          route_done[r2] = 1;
        }
        sb_append(&out, "        </Route>\n");
...
static void attach_root_lazies_ir(SolidProject *proj, IrNode *root) {
  if (!root) return;
  for (size_t i = 0; i < root->n_kids; i++) {
    IrNode *c = root->kids[i];
...
int solid_emit_modules_from_ir(IrProgram *ir, SolidWriteFn write_fn,
  ...
  SolidProject *proj = calloc(1, sizeof(SolidProject));
  if (!proj) return -1;
...
static char *solid_generate_impl_ir(IrProgram *ir) {
  if (!ir || !ir->root)
...
char *solid_generate_from_ir(IrProgram *ir) {
  ...
  if (!ir || !ir->root)
    return strdup("export default function App(){return null}\n");
  return solid_generate_impl_ir(ir);
}
