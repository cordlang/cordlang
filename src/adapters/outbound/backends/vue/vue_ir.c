...
/*
 * IR-2 pure Vue codegen: walks IrNode only for body / module emission.
...
static void emit_vue_prop_value(StrBuf *sb, const char *val) {
  if (!val) {
    sb_append(sb, "undefined");
    return;
  }
  if (irw_looks_number(val) || irw_looks_bool(val)) {
    /* caller prefixes with :name= */
    sb_appendf(sb, "\"%s\"", val);
    return;
  }
...
static char g_vu_stores[32][64];
static int g_vu_n_stores;

static void vu_reset_stores(void) { g_vu_n_stores = 0; }

static void vu_add_store(const char *name) {
  if (!name || !*name || g_vu_n_stores >= 32) return;
  for (int i = 0; i < g_vu_n_stores; i++)
    if (strcmp(g_vu_stores[i], name) == 0) return;
  snprintf(g_vu_stores[g_vu_n_stores], sizeof(g_vu_stores[0]), "%s", name);
  g_vu_n_stores++;
}

static int vu_is_store_expr(const char *expr) {
  if (!expr || !*expr) return 0;
  for (const char *p = expr; *p; p++) {
    if (!(isalnum((unsigned char)*p) || *p == '_' || *p == '$')) return 0;
  }
  for (int i = 0; i < g_vu_n_stores; i++)
    if (strcmp(g_vu_stores[i], expr) == 0) return 1;
  return 0;
}

static int is_vue_directive_attr(const char *name) {
  return name && (strcmp(name, "use") == 0 || strcmp(name, "transition") == 0 ||
                  strcmp(name, "in") == 0 || strcmp(name, "out") == 0 ||
                  strcmp(name, "animate") == 0);
}

static void emit_vue_directive(StrBuf *sb, const char *dir, const char *val) {
...
static int is_vue_transition_fn(const char *name) {
...
static int is_vue_animate_fn(const char *name) {
...
static void vu_needs_add_trans(VuModuleNeeds *n, const char *fn) {
  if (!n || !fn || !*fn || !is_vue_transition_fn(fn)) return;
...
static void vu_needs_add_anim(VuModuleNeeds *n, const char *fn) {
  if (!n || !fn || !*fn || !is_vue_animate_fn(fn)) return;
...
typedef enum { SK_PAGE = 0, SK_COMPONENT = 1, SK_LAYOUT = 2 } VueKind;

#define VU_MAX_UNITS 64
#define VU_MAX_USED 24
#define VU_MAX_ROUTES 64
...
typedef struct {
  VueUnit units[VU_MAX_UNITS];
  int n_units;
  IrNode *routes[VU_MAX_ROUTES];
  int n_routes;
  int has_router;
...
static int project_is_lazy_route_name(VueProject *proj, const char *name) {
...
static int route_is_lazy_ir(VueProject *proj, IrNode *route) {
  if (!route) return 0;
...
static void scan_deps_ir(IrNode *n, VueUnit *u) {
  if (!n || !u) return;
...
static void harvest_contexts_ir(VueProject *p, IrNode *root) {
  if (!p) return;
  if (root) {
...
static VueUnit *find_unit(VueProject *p, const char *name) {
  if (!name) return NULL;
  for (int i = 0; i < p->n_units; i++)
...
static void project_partition_from_ir(VueProject *p, IrNode *root) {
  memset(p, 0, sizeof(*p));
  if (!root) return;

  IrNode *raw_comps[VU_MAX_UNITS];
  ...
  for (size_t i = 0; i < root->n_kids; i++) {
    ...
    if (c->kind == IR_COMPONENT) {
      if (n_comp < VU_MAX_UNITS) raw_comps[n_comp++] = c;
      else p->truncated = 1;
    } else if (c->kind == IR_LAYOUT) {
      if (n_layout < 16) raw_layouts[n_layout++] = c;
      else p->truncated = 1;
    } else if (c->kind == IR_ROUTE) {
      if (p->n_routes < VU_MAX_ROUTES) p->routes[p->n_routes++] = c;
      else p->truncated = 1;
  ...
  for (int i = 0; i < n_comp; i++) {
    if (p->n_units >= VU_MAX_UNITS) {
      p->truncated = 1;
      break;
    }
    VueUnit *u = &p->units[p->n_units++];
    memset(u, 0, sizeof(*u));
    ...
    {
      char relbuf[128];
      snprintf(relbuf, sizeof(relbuf), "%s/%s.vue", u->dir, u->name);
  ...
  for (int i = 0; i < n_layout; i++) {
    ...
    {
      char relbuf[128];
      snprintf(relbuf, sizeof(relbuf), "layouts/%s.vue", u->name);
      snprintf(u->rel, sizeof(u->rel), "%s", relbuf);
    }
    cord_guess_source_path(u->name, u->dir, u->source_path,
                           sizeof(u->source_path));
  ...
  if (p->truncated) {
    fprintf(stderr,
            "error: project exceeds Vue backend limits "
            "(max %d units, %d routes) — split the app or raise limits\n",
            VU_MAX_UNITS, VU_MAX_ROUTES);
  }
}
...
static void emit_interp_expr(StrBuf *sb, const char *expr) {
  ...
  /* Vue template: refs auto-unwrap; stores are refs too */
...
static void gen_element_ir(StrBuf *sb, IrNode *node, int depth, int is_layout) {
  ...
  if (strcmp(tag, "link") == 0) html = "router-link";
  ...
  for (size_t i = 0; i < node->n_kids; i++) {
    ...
    if (strcmp(c->name, "bind") == 0 && c->value) {
      sb_appendf(sb, " v-model=\"%s\"", c->value);
      {
        int has_name = 0;
    ...
    if (is_vue_directive_attr(c->name) && c->value) {
      emit_vue_directive(sb, c->name, c->value);
      continue;
    }
  ...
  for (size_t i = 0; i < node->n_kids; i++) {
    ...
    if (c->kind == IR_EVENT && c->name && c->value) {
      ...
      if (strcmp(c->name, "submit") == 0) {
        if (needs_arrow)
          sb_appendf(sb, " @submit.prevent=\"() => { %s; }\"", c->value);
        else
          sb_appendf(sb, " @submit.prevent=\"%s\"", c->value);
      } else if (needs_arrow) {
        sb_appendf(sb, " @%s=\"() => %s\"", c->name, c->value);
...
static void gen_children_ir(StrBuf *sb, IrNode *node, int depth, int is_layout) {
  for (size_t i = 0; i < node->n_kids; i++) {
    ...
    if (child->kind == IR_FOR) {
      ...
      sb_indent(sb, depth);
      if (key)
        sb_appendf(sb, "<template v-for=\"%s in (%s || [])\" :key=\"%s\">\n", var, list, key);
      else
        sb_appendf(sb, "<template v-for=\"%s in (%s || [])\" :key=\"%s\">\n", var, list, var);
...
static void vu_collect_binds_ir(IrNode *n, char names[][64], int *count,
                                int max) {
  if (!n || *count >= max) return;
  ...
  for (size_t i = 0; i < n->n_kids; i++)
    vu_collect_binds_ir(n->kids[i], names, count, max);
}

static int vu_state_exists_ir(IrNode *def, const char *name) {
...
static void props_emit_one(const char *name, const char *deflt, void *ud) {
  PropsEmitCtx *ctx = ud;
  if (!name) return;
  props_collect_name(name, deflt, ctx);
  sb_appendf(ctx->script, "  %s: {", name);
  if (deflt && *deflt) {
    sb_append(ctx->script, " default: ");
...
static void state_emit_one(const char *name, const char *init, void *ud) {
  StateEmitCtx *ctx = ud;
  if (!name) return;
  sb_appendf(ctx->script, "const %s = ref(", name);
...
static void state_emit_setter(const char *name, const char *init, void *ud) {
  ...
  sb_appendf(ctx->script,
             "function %s(v) { %s.value = typeof v === 'function' ? v(%s.value) : "
             "v; }\n",
             setter, name, name);
}

/* Full .vue module from IR */
static char *gen_vue_module_ir(VueProject *proj, VueUnit *u) {
  IrNode *def = u->ir_def;
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    IrNode *c = def->kids[i];
    if (c->kind == IR_STORE && c->name) vu_add_store(c->name);
  }
  ...
  {
    int need_ref = 0, need_computed = 0, need_watch = 0, need_provide = need_set_ctx,
        need_inject = need_get_ctx;
    for (size_t _i = 0; _i < def->n_kids; _i++) {
      IrNode *_c = def->kids[_i];
      if (_c->kind == IR_STATE || irw_hook_is(_c, "ref") || _c->kind == IR_FETCH ||
          irw_hook_is(_c, "action"))
        need_ref = 1;
      if (_c->kind == IR_COMPUTED) need_computed = 1;
      if (_c->kind == IR_EFFECT) need_watch = 1;
      if (_c->kind == IR_FETCH) need_watch = 1;
    }
    char binds_tmp[32][64];
    int n_bt = 0;
    vu_collect_binds_ir(def, binds_tmp, &n_bt, 32);
    if (n_bt > 0) need_ref = 1;
    if (need_ref || need_computed || need_watch || need_provide || need_inject) {
      sb_append(&script, "import { ");
      int first = 1;
      if (need_ref) { sb_append(&script, "ref"); first = 0; }
      if (need_computed) {
        if (!first) sb_append(&script, ", ");
        sb_append(&script, "computed");
        first = 0;
      }
      if (need_watch) {
        if (!first) sb_append(&script, ", ");
        sb_append(&script, "watchEffect");
        first = 0;
      }
      if (need_provide) {
        if (!first) sb_append(&script, ", ");
        sb_append(&script, "provide");
        first = 0;
      }
      if (need_inject) {
        if (!first) sb_append(&script, ", ");
        sb_append(&script, "inject");
      }
      sb_append(&script, " } from 'vue';\n");
    }
  }
  (void)needs;
  ...
  (void)need_children;
  if (has_props || params_decl) {
    PropsEmitCtx pctx;
    memset(&pctx, 0, sizeof(pctx));
    pctx.script = &script;
    sb_append(&script, "const props = defineProps({\n");
    ...
    if (params_decl && params_decl->value) {
      ...
      while (*p) {
        ...
        if (!dup) {
          ...
          sb_appendf(&script, "  %s: {},\n", name);
          props_collect_name(name, NULL, &pctx);
        }
      }
    }
    sb_append(&script, "});\n");
    /* expose prop names as locals for template ergonomics */
    for (int k = 0; k < pctx.n_prop_names; k++) {
      sb_appendf(&script, "const %s = props.%s;\n", pctx.prop_names[k],
                 pctx.prop_names[k]);
    }
  }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (c->kind == IR_COMPUTED && c->name) {
      sb_appendf(&script, "const %s = computed(() => (%s));\n", c->name,
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (irw_hook_is(c, "ref") && c->value)
      sb_appendf(&script, "const %s = ref(null);\n", c->value);
  }

  for (size_t i = 0; i < def->n_kids; i++) {
    IrNode *c = def->kids[i];
    if (irw_hook_is(c, "ctx") && c->value && c->value2) {
      sb_appendf(&script, "const %s = inject(%s);\n", c->value, c->value2);
    }
  }
  ...
  for (int i = 0; i < n_provides; i++) {
    ...
    sb_appendf(&script, "provide(%s, ", ctx_name);
    if (val) {
      if (irw_looks_number(val) || irw_looks_bool(val) ||
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    sb_appendf(&script, "const %s = ref(", c->name);
    if (c->value)
    ...
    sb_appendf(&script, "function %s(v) { %s.value = v; }\n", setter, c->name);
  }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    if (irw_hook_is(c, "id") && c->value)
      sb_appendf(&script, "const %s = undefined; /* id */\n", c->value);
  }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    sb_appendf(&script, "const %s = ref(%s);\n", st, init);
    sb_appendf(&script, "const %s = ref(false);\n", pending_name);
    sb_appendf(&script, "  async function %s(e) {\n", action_name);
    sb_append(&script, "    e.preventDefault();\n");
    sb_appendf(&script, "    %s.value = true;\n", pending_name);
    sb_append(&script, "    try {\n");
    sb_appendf(&script, "      %s.value = await %s(%s.value, new FormData(e.target));\n", st,
               fn, st);
    sb_append(&script, "    } finally {\n");
    sb_appendf(&script, "      %s.value = false;\n", pending_name);
    sb_append(&script, "    }\n");
    sb_append(&script, "  }\n");
  }
  ...
  for (size_t i = 0; i < def->n_kids; i++) {
    ...
    sb_appendf(&script, "    %sLoading.value = true;\n", nm);
    sb_appendf(&script, "    %sError.value = null;\n", nm);
    {
    ...
    sb_append(&script, "      })\n");
    sb_appendf(&script,
               "      .then((data) => { if (!cancelled) %s.value = data; })\n", nm);
    sb_appendf(&script,
               "      .catch((err) => { if (!cancelled) %sError.value = err; })\n",
               nm);
    sb_appendf(&script,
               "      .finally(() => { if (!cancelled) %sLoading.value = false; });\n",
               nm);
...

  {
    char binds[32][64];
    int n_binds = 0;
    vu_collect_binds_ir(def, binds, &n_binds, 32);
    for (int bi = 0; bi < n_binds; bi++) {
      if (vu_state_exists_ir(def, binds[bi])) continue;
      if (vu_is_store_expr(binds[bi])) continue;
      sb_appendf(&script, "const %s = ref('');\n", binds[bi]);
    }
  }
...

  {
    ...
    vu_collect_binds_ir(def, binds, &n_binds, 32);
    for (int bi = 0; bi < n_binds; bi++) {
      if (vu_state_exists_ir(def, binds[bi])) {
        int has_explicit = 0;
        for (size_t i = 0; i < def->n_kids; i++) {
          IrNode *c = def->kids[i];
...
static char *gen_contexts_module_ir(VueProject *proj) {
  if (!proj || proj->n_contexts == 0) return NULL;
  StrBuf out;
  sb_init(&out);
  sb_append(&out, "/* Cordlang context keys — use with provide/inject */\n");
  for (int i = 0; i < proj->n_contexts; i++) {
...
static VueUnit *find_layout_for_route_ir(VueProject *proj, IrNode *route) {
...
static char *gen_router_js_ir(VueProject *proj) {
  StrBuf out;
  sb_init(&out);
  if (!proj || proj->n_routes <= 0) {
    sb_append(&out, "export const router = null;\n");
    return out.buf;
  }
  sb_append(&out, "import { createRouter, createWebHashHistory } from 'vue-router';\n");
  for (int i = 0; i < proj->n_units; i++) {
    if (proj->units[i].kind == SK_LAYOUT)
      sb_appendf(&out, "import %s from './layouts/%s.vue';\n",
                 proj->units[i].name, proj->units[i].name);
  }
  int imported[VU_MAX_UNITS];
  ...
  for (int r = 0; r < proj->n_routes; r++) {
    ...
    for (int i = 0; i < proj->n_units; i++) {
      ...
      if (strcmp(proj->units[i].name, comp) == 0) {
        sb_appendf(&out, "import %s from './%s/%s.vue';\n",
  ...
  for (int r = 0; r < proj->n_routes; r++) {
    ...
    sb_appendf(&out, "const %s = () => import('./%s/%s.vue');\n", comp, dir, comp);
  }
  sb_append(&out, "\nconst routes = [\n");
  for (int r = 0; r < proj->n_routes; r++) {
    ...
    VueUnit *layout = find_layout_for_route_ir(proj, proj->routes[r]);
    if (layout)
      sb_appendf(&out,
                 "  { path: '%s', component: %s, meta: { layout: %s } },\n",
                 path, comp, layout->name);
    else
      sb_appendf(&out, "  { path: '%s', component: %s },\n", path, comp);
  }
  sb_append(&out, "];\n\n");
  sb_append(&out,
...
static char *gen_app_vue_ir(VueProject *proj) {
  StrBuf out;
  sb_init(&out);
  ...
  if (proj->n_routes > 0) {
    ...
    sb_append(&out, "<script setup>\n");
    sb_append(&out, "import { RouterView } from 'vue-router';\n");
    sb_append(&out, "import { computed } from 'vue';\n");
    sb_append(&out, "import { router } from './router.js';\n");
    sb_append(&out,
  ...
  /* No routes: root page component */
  if (proj->n_units > 0) {
    VueUnit *root = &proj->units[0];
    for (int i = 0; i < proj->n_units; i++) {
...
int vue_emit_modules_from_ir(IrProgram *ir, VueWriteFn write_fn,
  ...
  VueProject *proj = calloc(1, sizeof(VueProject));
  if (!proj) return -1;
  ...
  for (int i = 0; i < proj->n_units; i++) {
    char *mod = gen_vue_module_ir(proj, &proj->units[i]);
    if (!mod) {
      free(proj);
      return -1;
    }
  ...
  {
    char *rj = gen_router_js_ir(proj);
    if (rj) {
      int rc = write_fn("router.js", rj, userdata);
      free(rj);
      if (rc != 0) {
        free(proj);
        return rc;
  ...
  char *app = gen_app_vue_ir(proj);
  if (!app) {
    free(proj);
    return -1;
  }
  int rc = write_fn("App.vue", app, userdata);
  free(app);
  free(proj);
  return rc;
}

static char *vue_generate_impl_ir(IrProgram *ir) {
  ...
  {
    char *rj = gen_router_js_ir(proj);
    sb_append(&sb, "<!-- ===== src/router.js ===== -->\n");
    sb_append(&sb, rj ? rj : "");
    sb_append(&sb, "\n");
    free(rj);
  }
  char *app = gen_app_vue_ir(proj);
  sb_append(&sb, "<!-- ===== src/App.vue ===== -->\n");
...
char *vue_generate_from_ir(IrProgram *ir) {
  if (!ir || !ir->root) return strdup("<!-- empty -->\n");
  return vue_generate_impl_ir(ir);
}
